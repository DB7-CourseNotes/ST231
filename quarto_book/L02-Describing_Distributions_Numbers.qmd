...
- Numerical summaries are called parameters when they describe an entire population, and statistics when they describe a sample\lspace
- Since we typically will not collect data on the entire population, we use (sample) statistics to make statements (with some uncertainty) about what the (population) parameter is.\lspace
- Categorical variables are summarized by the count or proportion (percent) of the data in each category.\lspace
...
- If we took an individual at random, what's the "best guess" of their height?\lspace
...
- The distances to the mean are all -2 or 2
    - If we found the mean, we'd get 0! We need to make sure they're all positive.\lspace
- Alternative 1: Absolute value. 
    The average *absolute* distance to the mean is 2.\lspace
...
1. Draw out bar plots.\lspace
2. Set up the table and find the variance.\lspace
...
- The "centre" is trying to measure the most common value.  
    - Often, this is our best **prediction**.\lspace
