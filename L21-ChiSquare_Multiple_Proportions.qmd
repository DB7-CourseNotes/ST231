...
Larger sample size $\rightarrow$ larger value of *expected*. For the same difference between observed and *expected*, a larger sample size means a smaller p-value! A smaller p-value means we're more likely to reject the null, and rejecting a false null is power!

\pspace

This is the same as with means and proportions!!!

There's no "standard error" for Chi-Square tests, but there's still a concept of "larger sample means smaller variance in the sampling distribution"!
