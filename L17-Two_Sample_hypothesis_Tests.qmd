...
## Example 2: Birthweights (by hand)
...
1. Ointment: $H_0: \mu_{without-with} = 0$ versus and $H_A: \mu_{without-with} < 0$. Note that we can tell that it's `without-with` by looking at the means of the two groups and the sign of the t-statistic. 
    - $H_0: \mu_{female} - \mu_{male} = 0$ versus $H_A: \mu_{female} - \mu_{male} \ne 0$.
    - The ointment example had $\mu_{without - with}$ as a single number, whereas the penguins example used $\mu_{female} - \mu_{male}$. This is because the ointment example was a *one-sample t-test* since we were treating the set of differences as a single sample; the penguins example is a two-sample t-test.
</details>