...
In high school, you may have learned a line as $y = mx + b$. In statistics, we often use latin letters for estimates and greek letters for population parameters^[Because we think it makes us fancy. Note that the Baldi \& Moore textbook uses $a$, $b$, and $e$ for everything.]. The population line is thus:
...
The word "regression" means to go backward. I like to think that we are "going backward" to the population numbers from the sample values^[Actually, the word comes from "regressing to the mean", which comes from how children are closer to average height than their parents - they go back toward the mean. This is not important.]. Any situation where you are estimating a population parameter is technically a **regression**, but this terminology is not useful for this class. 
...
These are called the **least squares** estimates^[There are other ways to estimate these parameters, but they're outside the scope of this course. All regression lines that you see in the textbook and the notes will be least squares regression lines.]. The equation for $b$ is especially important!
